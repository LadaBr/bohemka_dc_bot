...
client.on('messageUpdate', async (oldMessage, newMessage) => {
    ...
    if (newMessage.author.id === botId) {
        ...
        cells.date.value = moment(data.date).format(DATE_FORMAT);
        cells.isoDate.value = moment(data.date).toISOString();
        ...
        for (const slot of raidComp) {
            for (let i = 0; i < data.players.length; i++) {
                ...
                if (slot.cell.value === spec || slot.j > 4) {
                    ...
                    if (!cell.value) {
                        cell.value = slot.j > 4 && spec ? player.playerName + " | " + spec.trim() : player.playerName;
                        cell.backgroundColor = hexToRgb(classColors[player.className]);
...
const clearOldRaids = (raids) => {
    for (const raid of raids) {
        if (raid.cells.date.value && (moment() - moment(raid.cells.isoDate.value)) > 0) {
            console.log("Deleting old raid", raid);
            clearRaid(raid);
        }
    }
}
...
client.login('ODkyNDk0MDQwNzkxMTQyNDAw.YVNt2Q.tJaaKsgYeuTi5uqhRd6hrti0m-A');
